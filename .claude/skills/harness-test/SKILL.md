...
如果 `docs/design/test/test-brief.md` 中的 API 端点清单 存在 → 必须启动后端 → curl 每个端点 → 检查真实 HTTP 状态码和响应体 → 停止后端。不能只读代码判断「应该能跑」。
...
读取 `docs/design/test/` 文件夹。**这是你的唯一信息源——不读 design.md，不读 dev/，不读 gate/，不读 SPEC。**

- `docs/design/test/test-brief.md` — 含 AC 清单、API 端点清单、NFR 验证清单、部署运行命令

另外读取 `CLAUDE.md`、`package.json`/`requirements.txt` 等提取实际编译测试命令。

### 第 2 步：确定编译命令（不能猜）

### 第 3 步：确定测试命令（不能猜）

### 第 4 步：确定 API 验证命令（如果 `docs/design/test/test-brief.md` 中的 API 端点清单 存在）
...
**以下行为视为越界：**
- ❌ 不说「测试通过，可以交付了」——只报告结果，交付决策由 PM 和用户做
- ❌ 不修改代码修 bug——发现失败 → 回退到 dev，不是自己修
- ❌ 不在 verify.sh 不通过时标记 test→completed
